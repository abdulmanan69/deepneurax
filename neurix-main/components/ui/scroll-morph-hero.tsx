...
export default function IntroAnimation({ data }: { data?: ScrollMorphHeroData }) {
    ...
    const [scatterPositions, setScatterPositions] = useState<{x:number, y:number, rotation:number, scale:number, opacity:number}[]>([]);

    useEffect(() => {
        setScatterPositions(displayImages.map(() => ({
